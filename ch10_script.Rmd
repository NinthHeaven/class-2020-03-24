...
```{r bootstrap}
mean_income <- rep_sample_n(train, size = 115, reps = 1000, replace = TRUE) %>%
  group_by(replicate) %>%
...
mean_income_treated <- rep_sample_n(train, size = 115, reps = 1000, replace = TRUE) %>%
...
treated_minus_control <- rep_sample_n(train, 115, reps = 1000, replace = TRUE) %>%
  group_by(replicate, treatment) %>%
  summarise(mean = mean(income)) %>%
  pivot_wider(names_from = treatment, values_from = mean) %>%
  summarise(diff_income = Control - Treated) %>%
  pull(diff_income) %>%
...
mean_change <- train %>%
  ...
  summarise(diff_avg_att = Control - Treated)
```
...
```{r conf_att_chg}
treat_control_att_change <- rep_sample_n(train, 115, reps = 1000, replace = TRUE) %>%
  group_by(replicate, treatment) %>%
  summarise(avg_att = mean(att_chg)) %>%
  pivot_wider(names_from = treatment, values_from = avg_att) %>%
  summarise(diff_att_chg = Control - Treated) %>%
  pull(diff_att_chg) %>%
